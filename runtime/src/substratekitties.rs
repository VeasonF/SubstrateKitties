...
decl_storage! {
    trait Store for Module<T: Trait> as KittyStorage {
        ...
        BlindBoxKittiesArray get(kitty_in_blind_box): map (T::AccountId, u64) => T::Hash;
        BlindBoxKittiesCount get(blind_box_count): u64;
...
decl_module! {
    pub struct Module<T: Trait> for enum Call where origin: T::Origin {
        ...
        fn open_blind_box(origin) -> Result{
            let sender = ensure_signed(origin)?;

            let count_boxes = Self::blind_box_count();
            if(count_boxes < 3){
     ...
     qed");
        ...
        fn blind_box(origin, kitty_id: T::Hash)->Result{
...
impl<T: Trait> Module<T> {
	fn mint(to: T::AccountId, kitty_id: T::Hash, new_kitty: Kitty<T::Hash, T::Balance>) -> Result {
        ...
        let owned_kitties_count = <OwnedKittiesCount<T>>::get(&to);

        let new_owned_kitties_count = owned_kitties_count.checked_add(1)
        ...
        <OwnedKittiesArray<T>>::insert((to.clone(), owned_kitties_count), kitty_id);
        <OwnedKittiesCount<T>>::insert(&to, new_owned_kitties_count);
